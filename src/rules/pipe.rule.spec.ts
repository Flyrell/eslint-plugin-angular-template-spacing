import { ruleName, ruleModule } from '@plugin/rules/pipe.rule';
import type { BoundAttribute } from '@plugin/models/pipe.model';
import type { TSESLint, TSESTree } from '@typescript-eslint/utils';
import type { BoundText } from '@plugin/models/interpolation.model';
...
const mockBoundAttribute = (source: string, start: [number, number, number], end: [number, number]): BoundAttribute & TSESTree.BaseNode => ({
...
describe('Pipe Rule', () => {
    ...
    beforeEach(() => {
        context = { report: jest.fn() } as unknown as TSESLint.RuleContext<string, []>;
    })
    ...
    describe('BoundText Listener', () => {
        ...
        describe.each([
            ...
            it.each([
                ...
                { interpolation: '<div>{{something| pipe}}</div>', location: mockLocation([0, 0], [0, 29]), loc: mockLocation([0, 16], [0, 16]) },
                { interpolation: '<div>{{something |pipe}}</div>', location: mockLocation([0, 0], [0, 29]), loc: mockLocation([0, 18], [0, 18]) },
        ...
        describe('whitespace = never', () => {
            ...
            it.each([
                ...
                { interpolation: '<div>{{something| pipe}}</div>', location: mockLocation([0, 0], [0, 29]), loc: mockLocation([0, 17], [0, 18]) },
                { interpolation: '<div>{{something |pipe}}</div>', location: mockLocation([0, 0], [0, 29]), loc: mockLocation([0, 16], [0, 17]) },
